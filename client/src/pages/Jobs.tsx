  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-primary">Jobs & Opportunities</h1>
        {role === 'alumni' && (
          <button 
            onClick={() => setPostOpen(true)}
            className="bg-primary text-white px-4 py-2 rounded-lg hover:bg-[#093d5c] transition-colors font-medium shadow-sm flex items-center gap-2"
          >
      ...
      {loading && <div className="text-sm text-primary">Loading…</div>}
      
      {!loading && jobs.length === 0 && (
        <div className="text-center py-12 text-primary bg-white rounded-xl border border-secondary">
      ...
      <ul className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {jobs.map(j => (
          <li key={j.id}>
            <IconCard title={j.title} src={j.image || ICON} />
            <div className="mt-2 px-1">
              ...
              {j.link && (
                <a 
                  ...
                  className="mt-3 inline-flex items-center justify-center w-full py-2 px-4 rounded-lg bg-primary/5 text-primary text-xs font-bold hover:bg-primary hover:text-white transition-all duration-200 border border-primary/10"
                >
                  Apply Now
                  <svg className="ml-1.5 w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                </a>
              )}
            </div>
          </li>
